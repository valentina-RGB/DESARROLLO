...
import { faEdit, faTrash, faPlus, faBoxOpen, faToggleOn, faToggleOff,faSyncAlt} from '@fortawesome/free-solid-svg-icons';
...
//   },[]);
  ...
  const handleToggleEstado = useCallback (async(id: number, estadoActual: number) =>{
    ...
    const indexActual = estadosPedido.findIndex(estado => estado.ID_estado_pedido === estadoActual);
  ...
  if (indexActual === -1) return;
   ...
   const siguienteEstado = estadosPedido[(indexActual + 1) % estadosPedido.length];

   const estadoActualDescripcion = estadosPedido.find(e => e.ID_estado_pedido === estadoActual)?.descripcion;
        ...
        <div>
          <button
            ...
            onClick={async () => {
              ...
              try {
                await api.put(`/pedidos/${id}`, { ID_estado_pedido: siguienteEstado.ID_estado_pedido });
                // Actualizamos el estado localmente
                setPedidos(pedidos.map(pedido =>
                  pedido.ID_pedido === id ? { ...pedido, ID_estado_pedido: siguienteEstado.ID_estado_pedido } : pedido
                ));
                toast.success('El estado del pedido ha sido cambiado a "Cancelado".');
              } catch (error) {
    ...
    ()  => [
      ...
      },
      {
        ...
        Cell: ({ cell, row }) => {
            ...
            const estado = estadosPedido.find(e => e.ID_estado_pedido === cell.getValue<number>());
            ...
            }[estado.descripcion] : 'tw-bg-gray-100 tw-text-gray-800';
            ...
            return (
              <div className="tw-flex tw-items-center">
                ...
                <button
                  onClick={() => handleToggleEstado(row.original.ID_pedido, cell.getValue<number>())}
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="tw-flex tw-justify-center tw-gap-2">
            <button onClick={() => handleModal('edit',row.original.ID_pedido)} className="tw-bg-blue-500 tw-text-white tw-rounded-full tw-p-2 tw-shadow-md tw-hover:bg-blue-600 tw-transition-all tw-duration-300">
